...
def attention_mul(rnn_outputs, att_weights):
    ...
    attn_vectors = None
    #print("***********\n")
    #print("run_outputs.size(0)等于：",rnn_outputs.size(0))
    for i in range(rnn_outputs.size(0)):
        h_i = rnn_outputs[i]  # h_i维度为(seq_length, hidden_size)
        #print("att_weights[i]的shape:",att_weights[i].shape)
        #print("h_i的shape:",h_i.shape)
        #print("a_i的shape:",att_weights[i].unsqueeze(1).expand_as(h_i))
        #print("\n**********")
